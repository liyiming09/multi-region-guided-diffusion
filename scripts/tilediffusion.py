...
from tile_methods.prompt2prompt import AttentionReplace, register_attention_control, AttentionManipulate, MultiRegionAttentionController
...
from tile_utils.clipseg import CLIPDensePredT
from tile_utils.guide_loss import NCELoss
...
class Script(modules.scripts.Script):
                    ...
                    for i in range(BBOX_MAX_NUM):
                        ...
                        with gr.Accordion(f'Region {i+1}', open=False, elem_id=f'MD-accordion-{tab}-{i}'):
                            with gr.Row(variant='compact'):
                                ...
                                blend_mode.change(fn=lambda x: gr_show(x in ['Foreground', 'EditedSource', 'InversionSource', 'RemovalForeground', 'EditedTarget', 'AdditionForeground']), inputs=blend_mode, outputs=feather_ratio, show_progress=False)

                                # add_feather_ratio = gr.Slider(label='AddFeather', value=0.2, minimum=0, maximum=1, step=0.05, visible=False, elem_id=f'MD-{tab}-{i}-addfeather')
                                # blend_mode.change(fn=lambda x: gr_show(x==BlendMode.ADDITIONFOREGROUND.value), inputs=blend_mode, outputs=add_feather_ratio, show_progress=False)
                            ...
                            key_prompt = gr.Text(show_label=False, placeholder='Key Prompt, will work for the guidance', max_lines=1, elem_id=f'MD-{tab}-{i}-key-prompt')
                            with gr.Row(variant='compact'):
                        ...
                        control = [e, x, y, w, h, prompt, neg_prompt, blend_mode, feather_ratio, seed, key_prompt]
    ...
    def create_sampler_hijack(
        ...
        self.flag_noise_inverse = flag_noise_inverse
        ...
        use_baseline = False
        delegate.use_null_text_recons = False
        # init everything done, perform sanity check & pre-computations
        delegate.init_done()
        default_whole_step = 0.5
        delegate.none_region_insource = False 
        if p.batch_size == 2:
        # init the prompt controller to do the attn control by prompt-to-prompt
        # prompts = [delegate.custom_bboxes[-1].prompt[0], delegate.custom_bboxes[-1].prompt[1]] 
            delegate.controller = MultiRegionAttentionController(tokenizer = delegate.sampler.sampler.model.cond_stage_model.tokenizer, device = devices.device)
            cross_steps = {'Background': 1.0, 'Foreground': 1.0, 'AdditionForeground': 0.0,'RemovalForeground': 0.0,'EditedPair': 0.6, 'Whole': 0.5}
            self_steps = {'Background': 0.5, 'Foreground': 0.7, 'AdditionForeground': 0.0,'RemovalForeground': 0.0, 'EditedPair': 0.3,'Whole': 0.2}
            
            if len(delegate.custom_bboxes) > 0:
                for bbox_id, bbox in enumerate(delegate.custom_bboxes):
                    delegate.controller.select_region(bbox_id)
                    cross_replace_steps=cross_steps[bbox.blend_mode.value]
                    self_replace_steps=self_steps[bbox.blend_mode.value]
                    if bbox.blend_mode.value in ['Whole', 'AdditionForeground','RemovalForeground']: 
                        if bbox.blend_mode.value == 'Whole': delegate.controller.whole_id = bbox_id
                        if bbox.blend_mode.value == 'AdditionForeground': 
                            default_whole_step = 0.6
                            delegate.none_region_insource = False 
                            delegate.opti_source = False
                        elif bbox.blend_mode.value == 'RemovalForeground': 
                            default_whole_step = 0.6
                            delegate.none_region_insource = True
                            delegate.opti_source = True
                        delegate.controller.make_manipulate(bbox.batch_prompts, p.steps, cross_replace_steps=cross_replace_steps, self_replace_steps=self_replace_steps)
                    elif bbox.blend_mode.value in ['Background', 'Foreground', 'EditedPair']:
                        delegate.controller.make_replace(bbox.batch_prompts, p.steps, cross_replace_steps=cross_replace_steps, self_replace_steps=self_replace_steps)
                        if bbox.blend_mode.value == 'EditedPair': delegate.none_region_insource = True
            if not use_baseline:
                register_attention_control(delegate.sampler.sampler.model.model.diffusion_model, delegate.controller)
            # delegate.sampler.sampler.model.model.diffusion_model.requires_grad_(True).eval().to(devices.device)
            delegate.sampler.sampler.model.controller = delegate.controller

            # init the segmodel in the attn-guidance 
            ...
            segmodel.load_state_dict(torch.load('./extensions/multidiffusion-upscaler-for-automatic1111/tile_utils/weights/rd64-uni.pth', map_location=devices.device), strict=False)
            delegate.guidemodel = segmodel.to(devices.device)
            delegate.guide_loss = NCELoss()
            delegate.bgpreserve_loss = torch.nn.MSELoss(reduction='mean')
            delegate.get_crs_attn = True
            delegate.bg_rgb = True
            delegate.addi_region_words = {0:{}, 1:{}}
            delegate.grad_scale = [1000, 2000, 300]
            # delegate.guide_step = [1, 0.7, 0.8, default_whole_step]
            delegate.key_words_nums = 15
            delegate.map_bank = {}
            delegate.metric = 'entropy1d'
            delegate.controller.self_attn_scale = 1.5
            delegate.controller.key_self_attn_layer = ['down']
            delegate.flag_noise_inverse = flag_noise_inverse
            if flag_noise_inverse:
                delegate.none_region_insource = True
                delegate.grad_scale[2] = 100
            set_requires_grad(delegate.guidemodel, True)
            delegate.inversion_compare = False
            delegate.kssa = True
            delegate.use_null_text_recons = False
            delegate.soft_preserve = True
        if use_baseline:
            delegate.guide_step = [-1,-1,-1,-1]
        else:
            delegate.guide_step = [1, 0.7, 0.8, default_whole_step]
    ...
    def create_random_tensors_hijack(
        ...
        for i, v in bbox_settings.items():
            ...
            # elif BlendMode(v.blend_mode) == BlendMode.FOREGROUND or BlendMode(v.blend_mode) == BlendMode.ADDITIONFOREGROUND or BlendMode(v.blend_mode) == BlendMode.WHOLE:

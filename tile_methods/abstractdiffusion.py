...
class TiledDiffusion:
    ...
    def aggragate_prompts(self, total_prompt_words: List[str], prompt: str, total_prompt_region_index: List[int], region_id: int):
        new_words = prompt.split(',')
        for word in new_words:
            # if word not in total_prompt_words: 
            if len(total_prompt_words) == 0 or (word != '' or total_prompt_words[-1] != '' ):
                total_prompt_words.append(word)
                total_prompt_region_index.append(region_id)
    ...
    def init_custom_bbox(self, bbox_settings:Dict[int,BBoxSettings], draw_background:bool, causal_layers:bool):
        ...
        for bbox_setting in bbox_settings.values():
            e, x, y, w, h, p, n, blend_mode, feather_ratio, seed, kp = bbox_setting
            ...
            w = min(self.w - x, w)
            h = min(self.h - y, h)
            if blend_mode == 'EditedSource':
                edit_pair['src'] = [p, n, kp]
                if 'src' in edit_pair.keys() and 'target' in edit_pair.keys() :
                    self.custom_bboxes.append(EditBBox(x, y, w, h, edit_pair, BlendMode.EDITEDPAIR.value, feather_ratio, seed))
            elif blend_mode == 'EditedTarget':
                edit_pair['target'] = [p, n, kp]
                if 'src' in edit_pair.keys() and 'target' in edit_pair.keys() :
                    self.custom_bboxes.append(EditBBox(x, y, w, h, edit_pair, BlendMode.EDITEDPAIR.value, feather_ratio, seed))
            else:
                self.custom_bboxes.append(CustomBBox(x, y, w, h, p, n, blend_mode, feather_ratio, seed, kp))
        ...
        # prepare cond
        p = self.p
        total_prompt_words, total_prompt_regionids = [[],[]], [[],[]]
        total_neg_prompt_words, total_neg_prompt_regionids = [[],[]], [[],[]]
        total_key_prompt_words, total_key_prompt_regionids = [[],[]], [[],[]]
        ...
        for bbox_id, bbox in enumerate(self.custom_bboxes):
            #对prompt2prompt的整体区域进行上述bbox的操作，先进行信息聚合，再产生cond和uncond信息
            if bbox.blend_mode.value in ['Foreground', 'Background', 'RemovalForeground']:
                self.aggragate_prompts(total_prompt_words[0], bbox.prompt,total_prompt_regionids[0], bbox_id)
                self.aggragate_prompts(total_neg_prompt_words[0], bbox.neg_prompt, total_neg_prompt_regionids[0], bbox_id)
                self.aggragate_prompts(total_key_prompt_words[0], bbox.key_prompt, total_key_prompt_regionids[0], bbox_id)
            if bbox.blend_mode.value in ['Foreground', 'Background', 'AdditionForeground']:
                self.aggragate_prompts(total_prompt_words[1], bbox.prompt, total_prompt_regionids[1], bbox_id)
                self.aggragate_prompts(total_neg_prompt_words[1], bbox.neg_prompt, total_neg_prompt_regionids[1], bbox_id)
                self.aggragate_prompts(total_key_prompt_words[1], bbox.key_prompt, total_key_prompt_regionids[1], bbox_id)
            if bbox.blend_mode.value in ['EditedPair', 'InversionPair']:
                self.aggragate_prompts(total_prompt_words[0], bbox.prompt[0],total_prompt_regionids[0], bbox_id)
                self.aggragate_prompts(total_neg_prompt_words[0], bbox.neg_prompt[0], total_neg_prompt_regionids[0], bbox_id)
                self.aggragate_prompts(total_key_prompt_words[0], bbox.key_prompt[0], total_key_prompt_regionids[0], bbox_id)

                self.aggragate_prompts(total_prompt_words[1], bbox.prompt[1], total_prompt_regionids[1], bbox_id)
                self.aggragate_prompts(total_neg_prompt_words[1], bbox.neg_prompt[1], total_neg_prompt_regionids[1], bbox_id)
                self.aggragate_prompts(total_key_prompt_words[1], bbox.key_prompt[1], total_key_prompt_regionids[1], bbox_id)

        total_prompts, total_neg_prompts, total_key_prompts = [','.join(item) for item in total_prompt_words], [','.join(item) for item in total_neg_prompt_words], [','.join(item) for item in total_key_prompt_words]
        ...
        self.custom_bboxes.append(PromptToPromptBBox(x, y, w, h, total_prompts, total_neg_prompts, total_prompt_regionids, total_neg_prompt_regionids, BlendMode.WHOLE.value, feather_ratio, seed, total_key_prompts, total_key_prompt_regionids))
        ############################################done
        for bbox in self.custom_bboxes:
            bbox.cond, bbox.extra_network_data, bbox.batch_prompts = Condition.get_custom_cond(prompts, bbox.prompt, p.steps, p.styles, bbox.blend_mode)
            bbox.uncond, bbox.batch_neg_prompts = Condition.get_uncond_editversion(neg_prompts, bbox.neg_prompt, p.steps, p.styles, bbox.blend_mode)
    ...
    def reconstruct_custom_cond(self, org_cond:CondDict, custom_cond:Cond, custom_uncond:Uncond, bbox:CustomBBox) -> Tuple[List, Tensor, Uncond, Tensor]:
        ...
        tensor = Condition.reconstruct_cond(custom_cond, self.sampler.step)  #此处的step从0到20,在inversion中需要修正
    ...
    @custom_bbox
    def ddim_custom_forward(self, x:Tensor, cond_in:CondDict, bbox:CustomBBox, ts:Tensor, forward_func:Callable, bbox_id = 0, *args, **kwargs) -> Tensor:
        ''' draw custom bbox '''
        ...
        # 在此处完成uncond部分内容inversion的替换
        if self.flag_noise_inverse and bbox_id in self.opti_unconds[self.p.steps -1 - kwargs['index']].keys():
            if self.inversion_compare:
                uncond[1,...] = self.opti_unconds[self.p.steps -1 - kwargs['index']][bbox_id][0]
            else:
                uncond = self.opti_unconds[self.p.steps -1 - kwargs['index']][bbox_id][0].repeat(uncond.shape[0],1,1)
        ...
        return forward_func(x, cond, ts, unconditional_conditioning=uncond, *args, **kwargs)  # lead to the p_sample_ddim in ddim.py
    ...
    def init_noise_inverse(self, steps:int, retouch:float, get_cache_callback, set_cache_callback, renoise_strength:float, renoise_kernel:int):
        ...
        if self.sample_img2img_original is None:
            self.sample_img2img_original = self.sample_img2img_nulltext
    ...
    @noise_inverse
    def sample_img2img_nulltext(self, p, x, noise, conditioning, unconditional_conditioning, steps=None, image_conditioning=None, latents = None, t_enc = 0):
        if latents == None:
            x1 = self.sampler.sampler.stochastic_encode(x, torch.tensor([t_enc] * int(x.shape[0])).to(shared.device), noise=noise)
        else:
            x1 = latents[t_enc]['total'].repeat(noise.shape[0],1,1,1)

        self.sampler.init_latent = x
        self.sampler.last_latent = x
        self.sampler.step = 0
        ...
        samples = self.sampler.launch_sampling(t_enc + 1, lambda: self.sampler.sampler.decode(x1, conditioning, t_enc, unconditional_guidance_scale=p.cfg_scale, unconditional_conditioning=unconditional_conditioning))
    ...
    def sample_img2img(self, sampler: KDiffusionSampler, p:ProcessingImg2Img, 
        ...
        steps = self.sampler.adjust_steps_if_invalid(p, steps)
        self.sampler.initialize(p)

        self.sampler.sampler.make_schedule(ddim_num_steps=steps, ddim_eta=self.sampler.eta, ddim_discretize=p.ddim_discretize, verbose=False)
        latent_name = 'inversion-real-fruit1.pth'# 'inversion-people1.pth'#'inversion-fruit5.pth' #'inversion-add-apple-bg-w0.5.pth'# 'inversion-reverse-apple-bg-w0.5.pth'  'inversion-fruit5.pth'
        if cal_inversion:
            all_latents, opti_unconds = self.find_noise_for_ddim_inversion(self.sampler.sampler, self.noise_inverse_steps, noise, prompts)
            save_inversion = {'latents':all_latents, 'uncond':opti_unconds}
            torch.save(save_inversion, latent_name)
        else:
            save_inversion = torch.load(latent_name)
            all_latents, opti_unconds = save_inversion['latents'], save_inversion['uncond']
        shared.state.nextjob()
        latent = all_latents[-1]['total']
        ...
        self.opti_unconds = opti_unconds
        ...
        renoise_mask = torch.zeros((2, 1, noise.shape[2], noise.shape[3]), device=noise.device)
        inverse_noise = self.sampler.sampler.stochastic_encode(x, torch.tensor([p.steps-1] * int(x.shape[0])).to(shared.device), noise=noise)
        x0 = x.clone()
        if renoise_mask is not None:
            ...
            if not self.enable_grid_bbox:
                background_count  = torch.zeros((2, 1, noise.shape[2], noise.shape[3]), device=noise.device)
                foreground_noise  = torch.zeros_like(noise)
                foreground_weight = torch.zeros((2, 1, noise.shape[2], noise.shape[3]), device=noise.device)
                foreground_count  = torch.zeros((2, 1, noise.shape[2], noise.shape[3]), device=noise.device)
                for bbox in self.custom_bboxes:
                    if bbox.blend_mode == BlendMode.BACKGROUND:
                        background_count[bbox.slicer] += 1
                    else:
                    # elif bbox.blend_mode == BlendMode.FOREGROUND or bbox.blend_mode == BlendMode.ADDITIONFOREGROUND:
                        if bbox.blend_mode in [BlendMode.REMOVALFOREGROUND, BlendMode.ADDITIONFOREGROUND]:
                            src_slicer = list(bbox.slicer)
                            mask_slicer = tuple([slice(0,1)] + src_slicer[1:])
                            noise_slicer = tuple([slice(1,2)] + src_slicer[1:])

                            renoise_mask[noise_slicer] = bbox.feather_mask
                            x0[noise_slicer] = (1 - bbox.feather_mask) * x0[mask_slicer]
                            foreground_noise[noise_slicer] += noise[noise_slicer]
                            foreground_weight[noise_slicer] += bbox.feather_mask
                            foreground_count[noise_slicer] += 1
                        elif self.none_region_insource and bbox.blend_mode == BlendMode.EDITEDPAIR:
                            src_slicer = list(bbox.slicer)
                            mask_slicer = tuple([slice(0,1)] + src_slicer[1:])
                            noise_slicer = tuple([slice(1,2)] + src_slicer[1:])

                            renoise_mask[noise_slicer] = 0.5*bbox.feather_mask
                            foreground_noise[noise_slicer] += noise[noise_slicer]
                            foreground_weight[noise_slicer] += bbox.feather_mask
                            foreground_count[noise_slicer] += 1
                        else:
        ...
        return self.sample_img2img_original(p, x0, combined_noise, conditioning, unconditional_conditioning, steps, image_conditioning, t_enc = t_enc)


    @noise_inverse
    def find_noise_for_ddim_inversion(self, ddim_sampler, steps, noise, prompts:List[str]) -> Tensor:
        '''
        ...
        assert self.p.sampler_name == 'DDIM'

        latent = self.p.init_latent
        latent = latent.clone().detach()
        total_num = len(self.custom_bboxes)
        all_latents = [{'total':latent, total_num-1:latent}]
        b = latent.shape[0]

        image_conditioning = torch.zeros((b,5,1,1), device=latent.device)

        shared.state.sampling_steps = steps
        # ddim_sampler.make_schedule(ddim_num_steps=steps, ddim_eta=0.0, verbose=False)

        timesteps =  ddim_sampler.ddim_timesteps
        time_range = timesteps
        total_steps = timesteps.shape[0]
        iterator = tqdm(time_range, desc='Null-text DDIM Inversion', total=total_steps)
        
        # set_requires_grad(self.sampler.sampler.model.model.diffusion_model, True)
        for i, step in enumerate(iterator):
            # index = total_steps - i - 1 #19 --> 0
            index = i # 0 --> 19
            if self.use_null_text_recons:
                ts = torch.full((b,), step, device=latent.device, dtype=torch.long)

                cond = Condition.reconstruct_cond(self.cond_basis, total_steps - 1 - index)  #此处的step从0到20,在inversion中需要修正
                cond_in = self.make_condition_dict([cond], image_conditioning)
                inversion_latents = self.ddim_inversion(latent, cond_in, ts,
                                        index = index, is_inversion = True,
                                        optimization = False, unconditional_guidance_scale = self.p.cfg_scale)
                latent = inversion_latents['total']
                all_latents.append(inversion_latents)
                sd_samplers_common.store_latent(latent)
                # This is neccessary to save memory before the next iteration
                del cond, cond_in, ts
            else:
                inversion_latents = {}
                latent = self.sampler.sampler.stochastic_encode(latent, torch.tensor([index] * int(latent.shape[0])).to(shared.device), noise=noise)
                for bbox_id, bbox in enumerate(self.custom_bboxes):
                    inversion_latents[bbox_id] = latent[bbox.slicer]
                inversion_latents['total'] = latent 
                all_latents.append(inversion_latents)
                sd_samplers_common.store_latent(latent)
                # This is neccessary to save memory before the next iteration
                del  inversion_latents

            # iterator.update(1)
        iterator.close()
        # set_requires_grad(self.sampler.sampler.model.model.diffusion_model, False)
        time_range = np.flip(timesteps)
        iterator = tqdm(time_range, desc='Null-text DDIM optimization', total=total_steps)
        
        latent_cur = all_latents[-1]['total']
        opti_unconds = []
        for i, step in enumerate(iterator):
            index = total_steps - i - 1 #19 --> 0
            # index = i # 0 --> 19
            ...
            # parameter need to be convert to the ddim_inversion:
            #   unconditional_conditioning
            #   unconditional_guidance_scale
            cond = Condition.reconstruct_cond(self.cond_basis, total_steps - 1 - index)  #此处的step从0到20,在inversion中需要修正
            cond_in = self.make_condition_dict([cond], image_conditioning)
            latent_cur, pred_x0, uncond_embeddings_list = self.ddim_inversion(latent_cur, cond_in,
                                         ts, index = index, is_inversion = False,
                                         optimization = True,unconditional_guidance_scale = 7.5,
                                         latent_prev = latent_prev)
            opti_unconds.append(uncond_embeddings_list)
        ...
        return all_latents, opti_unconds
    ...
    @noise_inverse  #@torch.no_grad()
    def ddim_inversion(self, x_in: Tensor, sigma_in:Tensor, cond_in:Dict[str, Tensor], step:int) -> Tensor:
        pass
